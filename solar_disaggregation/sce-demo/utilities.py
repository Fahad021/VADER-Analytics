...
#        print('No init required')
    ...
    def QueryOrLoad(self,start_date = '01-01-2015', end_date = '01-01-2017'):
        ...
        initial_path = '' if path.exists('keys/pecanstkey.txt') else '../'
        fp = f'{initial_path}data/netloadsolaridentify_{start_date}_{end_date}.csv'
        fw = f'{initial_path}data/weather_netloadsolaridentify_{start_date}_{end_date}.csv'
        ...
        # Read the keys
        with open(f'{initial_path}keys/pecanstkey.txt', 'r') as f:
            key = f.read().strip()
            f.close()

        # Mayank:
        engine = sq.create_engine(
            f"postgresql+psycopg2://{key}@dataport.pecanstreet.org:5434/postgres"
        )
